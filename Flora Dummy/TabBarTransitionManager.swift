...
class TabBarTransitionManager: NSObject, UIViewControllerAnimatedTransitioning
{
    ...
    func animatePresentationWithTransitionContext(transitionContext: UIViewControllerContextTransitioning)
    {
        ...
        case 2: //Explode in
            ...
            let fromViewSnapshot = presentingControllerView.snapshotViewAfterScreenUpdates(false)
        ...
        case 3: //Flip right-to-left or left-to-right
            ...
            let toViewSnapshots = OBJ_CDefinitions.createSnapshots(presentedControllerView, afterScreenUpdates: true)
            var flippedSectionOfToView = toViewSnapshots[reverse ? 0 : 1] as! UIView
            
            let fromViewSnapshots = OBJ_CDefinitions.createSnapshots(presentingControllerView, afterScreenUpdates: false)
        ...
        case 4: //Fold
            ...
            for var i = 0; i < folds; i++
            {
                ...
                let leftFromViewFold = OBJ_CDefinitions.createSnapshotFromView(presentingControllerView, afterUpdates: false, location:CGFloat(offset), left: true, withNumberOfFolds: Int32(folds))
                ...
                let rightFromViewFold = OBJ_CDefinitions.createSnapshotFromView(presentingControllerView, afterUpdates: false, location:CGFloat(offset) + foldWidth, left: false, withNumberOfFolds: Int32(folds))
                ...
                let leftToViewFold = OBJ_CDefinitions.createSnapshotFromView(presentedControllerView, afterUpdates: true, location: CGFloat(offset), left: true, withNumberOfFolds: Int32(folds))
                leftToViewFold.layer.position = CGPointMake(reverse == false ? size.width : 0.0, size.height/2)
                leftToViewFold.layer.transform = CATransform3DMakeRotation(CGFloat(M_PI_2), 0.0, 1.0, 0.0)
                toViewFolds.addObject(leftToViewFold)
                
                let rightToViewFold = OBJ_CDefinitions.createSnapshotFromView(presentedControllerView, afterUpdates: true, location: CGFloat(offset) + foldWidth, left: false, withNumberOfFolds: Int32(folds))
                rightToViewFold.layer.position = CGPointMake(reverse == false ? size.width : 0.0, size.height/2)
            ...
            UIView.animateWithDuration(transitionDuration(transitionContext), delay: 0.0, options: .AllowUserInteraction, animations: { () -> Void in
                ...
                for var i = 0; i < self.folds; i++
                {
                    ...
                    leftFromView.layer.position = CGPointMake(self.reverse == false ? 0.0 : size.width, size.height/2.0)
                    ...
                    rightFromView.layer.position = CGPointMake(self.reverse == false ? 0.0 : size.width, size.height/2.0)
