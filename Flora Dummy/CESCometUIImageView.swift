...
class CESCometUIImageView: UIView
{
    ...
    override init(frame: CGRect)
    {
        ...
        addConstraints(NSLayoutConstraint.constraintsWithVisualFormat("[wheel(37)]", options: NSLayoutFormatOptions(rawValue: 0), metrics: nil, views: ["wheel":wheel]))
        addConstraints(NSLayoutConstraint.constraintsWithVisualFormat("V:[wheel(37)]", options: NSLayoutFormatOptions(rawValue: 0), metrics: nil, views: ["wheel":wheel]))
    ...
    {
        if target.canPerformAction(action, withSender: self) == false
        {
            return
        }
    ...
    override func touchesEnded(touches: Set<UITouch>, withEvent event: UIEvent?)
    {
        ...
        if CGRectContainsPoint(self.bounds, point) && target != nil && action != nil
        {
            ...
            NSTimer.scheduledTimerWithTimeInterval(0.0, target: target!, selector: action!, userInfo: ["ImageView" : self], repeats: false)
        }
    }
    
    override func touchesMoved(touches: Set<UITouch>, withEvent event: UIEvent?)
    {
        let previousPoint = touches.first!.previousLocationInView(self)
        let point = touches.first!.locationInView(self)
        
        if CGRectContainsPoint(self.bounds, point)
        {
            if CGRectContainsPoint(self.bounds, previousPoint) == false
            {
