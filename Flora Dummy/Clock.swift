...
class Clock: UIView
{
    ...
    By default, this is set to 'true'.  Setting to 'No' disables the time-link between each hand.
    */
    var handsMoveDependently = true
    ...
    */
    ...
    private func makeHands()
    {
        ...
        hoursHand.layer.shouldRasterize = true
    ...
    func handleMinutesPan(panGesture : UIPanGestureRecognizer)
    {
        ...
        case .Changed:
            ...
            let adjacent = Float(pointOne.x - pointTwo.x)
            let opposite = Float(pointOne.y - pointTwo.y)
            ...
            UIView.animateWithDuration(0.1, delay: 0.0, options: .AllowAnimatedContent, animations: { () -> Void in
                ...
                if self.handsMoveDependently == true
                {
        ...
        case .Ended:
            if minuteHandRounding != .None
            {
                ...
                var minutes = Int(timeComponents[1])!
                ...
                UIView.animateWithDuration(0.5, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.0, options: .AllowAnimatedContent, animations: { () -> Void in
                    ...
                    let secondsValue = self.showSecondsHand == true ? (timeComponents[2] as NSString).doubleValue/60.0/60.0 : Double(0.0)
                    
                    let trueHour = CGFloat((timeComponents[0] as NSString).doubleValue + (timeComponents[1] as NSString).doubleValue/60.0 + secondsValue)
                    self.hoursHand!.transform = CGAffineTransformMakeRotation(CGFloat(M_PI * 2.0)/CGFloat(12.0) * trueHour)
                    
                    self.minutesHand!.transform = CGAffineTransformMakeRotation(CGFloat(M_PI * 2.0)/CGFloat(60.0) * CGFloat((timeComponents[1] as NSString).doubleValue + (self.showSecondsHand == true ? (timeComponents[2] as NSString).doubleValue/60.0 : 0.0)))
    ...
    - parameter time: The time to move to in "HH:MM:SS" format
    */
    func rotateHandsToTime(time : String, animated: Bool)
    {
        ...
        UIView.animateWithDuration(animated == true ? 1.0: 0.0, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.0, options: .AllowAnimatedContent, animations: { () -> Void in
